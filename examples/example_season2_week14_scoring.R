# ==============================================================================
# NFL Analytics Toolkit -- Season 2, Week 14
# Usage Examples: Final Prospect Scoring and Signal Tier CSV
# File: examples/example_season2_week14_scoring.R
#
# PURPOSE
#   Sources R/28 and runs the full scoring pipeline. Produces three output
#   files in data/season2_cache/:
#     s2_week14_final_prospect_scores.csv   -- 848-row prospect inspection CSV
#     s2_week14_tier_reference.csv          -- empirical tier boundary reference
#     s2_week14_metric_correlations.csv     -- per-position Pearson correlation matrix
#
# PREREQUISITES
#   R/27_translation_model_v3.R must have been run first and its RDS artifacts
#   present in data/season2_cache/ (named s2_week13_*). The Databricks marker
#   CSV must also be present at data/season2_cache/s2_week13_full_marker_scores.csv.
#
# EXECUTION ORDER (mandatory)
#   1. source(here::here("R", "27_translation_model_v3.R"))  -- if not already run
#   2. source(here::here("examples", "example_season2_week14_scoring.R"))
#
...
PATH_SCORES   <- here::here("data", "season2_cache", "s2_week14_final_prospect_scores.csv")
PATH_TIER_REF <- here::here("data", "season2_cache", "s2_week14_tier_reference.csv")
PATH_CORR     <- here::here("data", "season2_cache", "s2_week14_metric_correlations.csv")
...
for (pos in c("QB", "RB", "WR", "TE")) {
  ...
  top10 <- scores |>
    ...
    dplyr::select(
      cfb_player_name, draft_year, score_final,
      enriched_improvement_pct, score_v1
    ) |>
    dplyr::mutate(
...
top_wr_name <- scores |>
  ...
  dplyr::pull(cfb_player_name)

cat(glue("Top WR by score_final: {top_wr_name}\n\n"))

top_wr_row <- scores |>
  dplyr::filter(cfb_player_name == top_wr_name, position == "WR")
...
cat(glue(
  ...
  "score_v1:                 {round(top_wr_row$score_v1, 1)}\n",
...
cat(glue(
  ...
  "  When two metrics are correlated, a player clearing both thresholds\n",
  "  is not getting two independent signals -- review tier signals accordingly.\n\n"
))
