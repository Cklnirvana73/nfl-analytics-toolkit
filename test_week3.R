# ==============================================================================
# WEEK 3 TESTS - Fantasy Scoring & Rolling Averages
# ==============================================================================
#
# This file contains comprehensive tests for Week 3 functions:
...
#
# Run all tests with: source("tests/test_week3.R")
#
# ==============================================================================
...
test_that("Default settings use tiered PPR, TE premium, 6pt pass TDs", {
  ...
  # Use defaults except explicitly confirm what we're testing
  fantasy <- calculate_fantasy_points(mock_pbp,
                                      use_tiered_ppr = TRUE,
                                      ppr = 0)  # Tiered replaces regular PPR
...
test_that("Standard scoring (no PPR, 4pt pass TDs) works", {
  # Need 2 separate pass plays for 2 TDs (can't have 2 TDs on 1 play)
  mock_pbp <- tibble(
    season = 2024,
    week = 1,
    game_id = "2024_01_TEST",
  ...
  fantasy <- calculate_fantasy_points(
    ...
    rush_att_bonus = 0,
    ppr = 0,
    pass_td = 4  # Explicitly set 4pt pass TDs
  )
  
  # 300 yards * 0.04 = 12
  # 2 TDs * 4 = 8
  # Total = 20 pts
  expect_equal(fantasy$pass_fantasy_points[1], 20, tolerance = 0.01)
})
...
test_that("Bye weeks are handled correctly", {
  ...
  # Week 5 should use weeks 2, 3 (only 2 games before bye)
  expect_equal(rolling_stats$rush_yards_roll3[4], 100, tolerance = 0.01)
})
